...
const ameoraImages = [
  ...
  '/new_img/ameora/10.png',
  '/new_img/ameora/4.png',
  '/new_img/ameora/5.png',
  '/new_img/ameora/14.png',
  '/new_img/ameora/6.png',
  '/new_img/ameora/7.png',
  '/new_img/ameora/15.png',
];
    ...
    <div>
      <div className="relative h-[340px] sm:h-[400px] lg:h-[500px] rounded-3xl overflow-hidden shadow-2xl border border-[#C9A84C]/20 mb-3">
        <AnimatePresence mode="sync">
          <motion.img key={current} src={images[current]} alt="Améora"
      ...
      <div className="flex gap-1.5 sm:gap-2 overflow-x-auto pb-1 sm:grid sm:grid-cols-7">
        {images.map((img, i) => (
          <button key={i} onClick={() => setCurrent(i)}
  ...
  return (
    <div className="pt-16 bg-[#0D0D0D] min-h-screen">
      ...
      <section className="bg-gradient-to-br from-[#0D0D0D] via-[#1A1A1A] to-[#6B3F1F]/30 py-12 sm:py-20 lg:py-24 overflow-hidden relative border-b border-[#C9A84C]/20">
        <div className="absolute top-0 left-0 w-[300px] sm:w-[500px] h-[300px] sm:h-[500px] bg-[#C9A84C]/5 rounded-full blur-3xl" />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
            ...
            <div className="flex items-center gap-4 mb-6">
              <span className="text-2xl sm:text-3xl font-extrabold text-[#C9A84C]">₹1,099</span>
            </div>
      ...
      <section className="py-12 sm:py-16 bg-[#0D0D0D]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <p className="text-[#C9A84C]/50 text-sm">See Améora from every angle</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <ImageGallery images={ameoraImages} />
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-12 sm:py-16 bg-gradient-to-r from-[#6B3F1F] to-[#C9A84C]/80 text-center px-4">
        <h2 className="text-2xl sm:text-3xl font-heading font-bold text-[#0D0D0D] mb-4">Ready to Experience Améora?</h2>
      ...
      <div className="py-8 text-center max-w-4xl mx-auto px-4">
        <p className="text-xs text-[#FDF6E3]/20 leading-relaxed">
          Disclaimer: This product is formulated with natural extracts. Results may vary. For external use only. Consult a healthcare professional if needed.
        </p>
      </div>
    </div>
  );
